...
export class ORegister {
  ...
  async submit() {
    ...
    this.formResponse = await AuthenticationService
      .register(formDataObject as IRegisterAuthenticationRequest);

    if (this.formResponse.status === HttpStatus.CREATED) {
      this.scrollIntoView();
    } else {
      this.scrollIntoView();
  ...
  render() {
    return (
      <Host>
        ...
        >
          <c-section>
            <h1 class="grid__full">Registrieren</h1>
            <p class="grid__full">Hier können Sie sich kostenlos registrieren.</p>
          </c-section>

          <c-section>
            <a-input type="text" name="firstname" minlength={2} class="grid__left" required>
            ...
            <a-input type="text" name="lastname" minlength={2} class="grid__right" required>
